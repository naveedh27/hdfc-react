...
class Home extends Component {
    ...
    sortFilterDataByPercent = (_reverse) => {
        const sortedData = this.state.currFilertData.sort((a, b) => {
    ...
    sortFilterDataByDate = (_reverse) => {
        const sortedData = this.state.currFilertData.sort((a, b) => {
            const aDate = new Date(Date.parse(a[ 'end.time' ]))
            const bDate = new Date(Date.parse(b[ 'end.time' ]))
    ...
    async componentDidMount() {
        const _apiData = await axios.get('http://starlord.hackerearth.com/kickstarter')
